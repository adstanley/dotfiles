...
function _tmux_complete_client() {
    ...
    # Use mapfile to capture the output into an array
    mapfile -t new_entries < <(compgen -W "$(tmux "$@" list-clients -F '#{client_tty}' 2> /dev/null)" -- "${cur}")
    
    # Append the new entries to COMPREPLY
    COMPREPLY+=("${new_entries[@]}")
