...
# function hex2bin() { s=${@^^}; for i in $(seq 0 1 $((${#s}-1))); do printf "%04s" "$(printf "ibase=16; obase=2; %s;\n" "${s:$i:1}" | bc)" ; done; printf "\n"; }

function hex2bin() {
    local hex="${*^^}"  # Convert input to uppercase
    local bin=""
    local -A h2b=([0]="0000" [1]="0001" [2]="0010" [3]="0011"
...
function optpdf72() {
    local input_file="$1"
    local output_file="${input_file%.*}_small.pdf"
    
    mkdir -p "./temppdf" && 
    cd "./temppdf" && 
    pdftoppm -jpeg -r 72 "$input_file" pg && 
    img2pdf ./*.jpg --output "$output_file" && 
    mv "$output_file" .. &&
    cd .. && 
    rm -rf "./temppdf"
}
...
#
function img2pdfcheck(){ echo '' && echo 'Checking rotation of JPF images... Please be patient...' && n=0 && TOTAL="$(find . -iname "*.jpg" | wc | awk '{print $1}')" && for i in *.jpg; do (( n++ )) && echo "Checking file $i, which is $n out of $TOTAL" && if [ "$(img2pdf "$i" | grep 'Invalid')" != '' ]; then echo "$i - Rotation is bad"; fi ; done ; }
...
#
function dispcolor() { display -size 300x300 xc:"$1" ; }
function displaycolor() { dispcolor "$1" ; }
...
function daymode() { xgamma -rgamma 1 -ggamma 1 -bgamma 1; }
